...
- 本汇总基于 `artifacts/reports/*.json` 中已保存的 BUSI `pathology_label` 与 `malignant_probability` 重新计算。
- 这一步不重新训练模型、不选择模型、不改变权重，只修正评估报告字段，使其包含比赛要求的 AUC、Accuracy、Recall/Sensitivity、Precision、Specificity 和 F1-Score。
- BUSI 仍是锁定外部评估集；表中的 best Youden 只用于运行点分析，不应作为训练或调参依据。
- 当前 demo 主线已另行重新跑完整外部评估，详见 `artifacts/reports/competition_metrics_audit.md`。

- 已刷新 BUSI JSON 报告数量：`86`
