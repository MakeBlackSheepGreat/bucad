...
# Fold1 Single Model Baseline Performance Comparison
...
- This report uniformly evaluates locally available fold1 classification checkpoints, including mainline models, tuned models, and comparison models from comparison.yml.
- The evaluation set is BUSI; BUSI does not participate in training.
- The table shows single-model single-fold results, which do not represent 5-fold stability or final hybrid ensemble performance.
- AlexNet failed in existing comparison training records and has no checkpoint available for evaluation, so it is not included in the ranking table.

## BUSI External Evaluation: Default Threshold 0.50

| Rank | Model | AUC | Sens @0.50 | Spec @0.50 | Acc @0.50 | Precision | F1-Score | Confusion | Notes |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |
...
| Rank | Model | AUC | Threshold | Sens | Spec | Acc | Youden J | Precision | F1-Score | Confusion | Notes |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |
...
- Among fold1 single models, the highest BUSI AUC belongs to ConvNeXt-Tiny timm-aware fold1, with AUC `0.8943`.
- The timm-aware recipe clearly corrected the early training/preprocessing issues of ConvNeXt-Tiny and Swin-Tiny.
- A single fold1 is only suitable as a reference for model baseline potential; final selection should still prioritize 5-fold and external evaluation results.
